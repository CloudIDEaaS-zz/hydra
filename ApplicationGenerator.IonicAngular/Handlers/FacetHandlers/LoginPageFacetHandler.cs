...
namespace AbstraX.FacetHandlers
{
    ...
    public class LoginPageFacetHandler : BasePageFacetHandler
    {
        ...
        public override bool Process(IBase baseObject, Facet facet, IGeneratorConfiguration generatorConfiguration)
        {
            ...
            var identityFields = new List<IdentityField>();
            ...
            foreach (var childObject in baseElement.GetIdentityFields(IdentityFieldKind.UserName, IdentityFieldKind.PasswordHash))
            {
                if (childObject is IAttribute)
                {
                    identityFields.Add(new IdentityField((IAttribute)childObject, facet, generatorConfiguration));
                }
                else
                {
                    DebugUtils.Break();
            ...
            LoginPageGenerator.GeneratePage(baseObject, pagePath, name, generatorConfiguration, module, importGroups, identityFields, loginTitleTranslationKey, loginButtonTranslationKey, loadKind, kind);
