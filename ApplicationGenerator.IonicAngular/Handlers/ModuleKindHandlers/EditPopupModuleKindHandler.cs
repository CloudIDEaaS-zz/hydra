...
{
    [AngularModuleKindHandler(ModuleKind.AngularModule, DefinitionKind.Class, UIFeatureKind.EditPopup)]
    public class EditPopupModuleKindHandler : IModuleAddModuleKindHandler
    {
        public float Priority => 1.0f;
        public event ProcessFacetsHandler ProcessFacets;

        public EditPopupModuleKindHandler()
        {
        }
        ...
        public bool Process(Enum moduleKind, IModuleObject moduleObject, Folder folder, IGeneratorConfiguration generatorConfiguration)
        {
            ...
            EditModuleGenerator.GenerateModule(baseObject, angularModule, folder.fullPath, name, generatorConfiguration, imports, true, angularModule.UILoadKind, angularModule.UIKind);
