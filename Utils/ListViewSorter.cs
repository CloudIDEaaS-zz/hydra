...
namespace Utils
{
    ...
    public class ListViewColumnSorter : IComparer, IComparer<ListViewItem>
    {
        ...
        private ImageTextComparer TextObjectCompare;
        private CheckboxTextComparer CheckBoxObjectCompare;
        public IComparer CustomCompare { get; set; }

        private SortModifiers sortModifier = SortModifiers.SortByText;
        ...
        public ListViewColumnSorter()
        {
            ...
            TextObjectCompare = new ImageTextComparer();
            CheckBoxObjectCompare = new CheckboxTextComparer();
        }
        ...
        public int Compare(object x, object y)
        {
            ...
            if (listViewMain != null && (listViewMain.Sorting != SortOrder.Ascending && listViewMain.Sorting != SortOrder.Descending))
            {
                // Return '0' to indicate they are equal
                return compareResult;
            }

            if (sortModifier.Equals(SortModifiers.SortByText) || ColumnToSort > 0)
            {
            ...
            else
            {
                switch (sortModifier)
                {
                    case SortModifiers.SortByCheckbox:
                        compareResult = CheckBoxObjectCompare.Compare(x, y);
                        break;
                    case SortModifiers.SortByImage:
                        compareResult = TextObjectCompare.Compare(x, y);
                        break;
                    case SortModifiers.SortCustom:
                        compareResult = CustomCompare.Compare(x, y);
                        break;
                    default:
                        compareResult = TextObjectCompare.Compare(x, y);
                        break;
