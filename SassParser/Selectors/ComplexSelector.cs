...
namespace SassParser
{
    internal sealed class ComplexSelector : StylesheetNode, ISelector
    {
        ...
        public ComplexSelector(Token token) : base(token)
        {
        ...
        public void AppendSelector(ISelector selector, Combinator combinator, Token token)
        {
            if (!IsReady)
            {
                _selectors.Add(new CombinatorSelector
                {
                    Selector = combinator.Change(selector, token),
