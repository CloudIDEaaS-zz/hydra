...
namespace SassParser
{
    public sealed class StyleDeclaration : StylesheetNode, IProperties
    {
        ...
        private StyleDeclaration(Rule parent, Token token, StylesheetParser parser)  : base(token)
        {
            _parent = parent;
            _parser = parser;
        }

        internal StyleDeclaration(Token token, StylesheetParser parser) : this(null, token, parser)
        {
        }

        internal StyleDeclaration(Token token) : this(null, token, null)
        {
        }

        internal StyleDeclaration(Rule parent, Token token) : this(parent, token, parent.Parser)
        {
        }
            ...
            foreach (var declaration in Declarations)
            {
                ...
                if (IsStrictMode)
                {
                    ...
                    if (shorthands.Any())
                    {
                        ...
                        {
                            var rule = PropertyFactory.Instance.CreateShorthand(shorthand, null);
        ...
        public string GetPropertyValue(string propertyName)
        {
            ...
            var shortHand = PropertyFactory.Instance.CreateShorthand(propertyName, null);
        ...
        public void SetPropertyValue(string propertyName, string propertyValue, Token token = null)
        {
            SetProperty(propertyName, propertyValue, token);
        }
        ...
        public void SetProperty(string propertyName, string propertyValue, Token token, string priority = null)
        {
            if (!string.IsNullOrEmpty(propertyValue))
            {
                ...
                var property = CreateProperty(propertyName, token);
        ...
        internal Property CreateProperty(string propertyName, Token token)
        {
            var property = GetProperty(propertyName);
            if (property != null)
            {
            ...
            property = PropertyFactory.Instance.Create(propertyName, token);
            if (property != null || IsStrictMode)
            {
                return property;
            }

            return new UnknownProperty(propertyName, token);
        }
        ...
        {
            var properties = PropertyFactory.Instance.CreateLonghandsFor(shorthand.Name, null);
