...
namespace SassParser
{
    ...
    {
        public virtual ISelector Change(ISelector selector, Token token)
        {
            return selector;
        }
        ...
        private sealed class NamespaceCombinator : Combinator
        {
            ...
            public override ISelector Change(ISelector selector, Token token)
            {
                var prefix = selector.Text;
                return new SimpleSelector( /*el => el.MatchesCssNamespace(prefix),*/ Priority.Zero, prefix, token);
            }
        }
