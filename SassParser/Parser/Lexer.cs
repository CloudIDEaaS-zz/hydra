...
namespace SassParser
{
    internal sealed class Lexer : LexerBase
    {
        ...
        internal void RaiseErrorOccurred(ParseError error, string message, TextPosition position, int length)
        {
            var handler = Error;
            ...
            var errorEvent = new TokenizerError(error, message, position, length);
        ...
        private Token Data(char current)
        {
            ...
            switch (current)
            {
                ...
                case Symbols.ReverseSolidus:
                    ...
                    {
                        RaiseErrorOccurred(ParseError.LineBreakUnexpected, Strings.LineBreakUnexpected);
                    ...
                    RaiseErrorOccurred(ParseError.EOF, Strings.EOF);
                    return NewDelimiter(GetPrevious());
        ...
        private Token HashStart()
        {
            ...
            {
                RaiseErrorOccurred(ParseError.InvalidCharacter, Strings.InvalidCharacter);
                Back();
        ...
        private void RaiseErrorOccurred(ParseError code, string message)
        {
            RaiseErrorOccurred(code, message, GetCurrentPosition(), 0);
        }
    }
}