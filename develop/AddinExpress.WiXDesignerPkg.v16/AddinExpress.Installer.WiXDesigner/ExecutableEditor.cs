...
{
	internal class ExecutableEditor : UITypeEditor, IWin32Window
	{
		...
		private object ShowDialog(VSBaseFolder parent, string fileName, object defaultValue)
		{
			...
			try
			{
				formSelectItemInProject.Initialize(parent, fileName, this.vsDialog.Project.FileSystem, this.vsDialog.Project.Binaries, new string[] { "Executable Files (*.exe)", "All Files (*.*)" });
				if (formSelectItemInProject.ShowDialog() == DialogResult.OK)
				{
					...
					if (string.IsNullOrEmpty(sourcePath) || !sourcePath.EndsWith(".exe", StringComparison.OrdinalIgnoreCase))
					{
