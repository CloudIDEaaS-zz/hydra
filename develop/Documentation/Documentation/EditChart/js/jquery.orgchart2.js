...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      if ($.type(data) === 'object') {
        if (data instanceof $) { // ul datasource
          this.buildHierarchy($chart, null, this.buildJsonDS(data.children()), 0, this.options);
        } else { // local json datasource
          this.buildHierarchy($chart, null, this.options.ajaxURL ? data : this.attachRel(data, '00'));
        }
      } else {
    ...
    buildHierarchy: function ($appendTo, parentData, childData) {
      var that = this;
      let newParent;
      var opts = this.options;
      var level = 0;
      if (childData.level) {
        ...
        level = childData.level = $appendTo.parentsUntil('.orgchart', '.nodes').length + 1;
      }
      // Construct the node
      var childrenData = childData.children;
      var hasChildren = childrenData ? childrenData.length : false;
      var $nodeWrapper;
      if (Object.keys(childData).length > 2) {
        var $nodeDiv = this.createNode(childData);
        if (opts.verticalLevel && level >= opts.verticalLevel) {
      ...
      if (hasChildren) {
        var isHidden = (level + 1 > opts.visibleLevel || childData.collapsed) ? ' hidden' : '';
        var childOrientationVertical = parentData == null ? false : parentData.nodeData.childOrientation == "vertical" ? true : false;
        // var isVerticalLayer = (opts.verticalLevel && (level + 1) >= opts.verticalLevel) ? true : childOrientationVertical;
...
//          if (childOrientationVertical || level + 1 === opts.verticalLevel) {
        ...
        $.each(childrenData, function () {
          ...
          this.level = level + 1;
          that.buildHierarchy($nodeCell, newParent, this);
        });
      }
    },
    // build the child nodes of specific node
    buildChildNode: function ($appendTo, parentData, childData) {
      $appendTo.find('td:first').attr('colspan', childData.length * 2);
      this.buildHierarchy($appendTo, parentData, { 'children': childData });
    },
    // exposed method
    addChildren: function ($node, parentData, childData) {
      this.buildChildNode($node.closest('table'), parentData, childData);
    ...
    buildSiblingNode: function ($nodeChart, parentData, childData) {
      var newSiblingCount = $.isArray(childData) ? childData.length : childData.children.length;
      ...
      if ($nodeChart.parent().is('td')) {
        ...
        this.buildChildNode($nodeChart.parent().closest('table'), parentData, childData);
        ...
        this.buildHierarchy($nodeChart.closest('.orgchart'), parentData, childData);
        this.complementLine($nodeChart.next().children('tr:last').children().eq(insertPostion).after($('<td colspan="2">').append($nodeChart)),
          siblingCount, 1);
      }
    },
    //
    addSiblings: function ($node, parentData, childData) {
      this.buildSiblingNode($node.closest('table'), parentData, childData);
