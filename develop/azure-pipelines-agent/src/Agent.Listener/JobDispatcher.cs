    ...
    public sealed class JobDispatcher : AgentService, IJobDispatcher
    {
        ...
        public override void Initialize(IHostContext hostContext)
        {
            ...
            if (!int.TryParse(Environment.GetEnvironmentVariable("VSTS_AGENT_CHANNEL_TIMEOUT") ?? string.Empty, out channelTimeoutSeconds))
            {
                ...
                channelTimeoutSeconds = int.MaxValue;
            }
        ...
        public async Task WaitAsync(CancellationToken token)
        {
            ...
            if (currentDispatch != null)
            {
                ...
                using (var registration = token.Register(() => { if (currentDispatch.Cancel(TimeSpan.FromSeconds(int.MaxValue))) { Trace.Verbose($"Fired cancellation token for job request {currentDispatch.JobId}."); } }))
                {
                    try
                    {
