...
namespace Polenter.Serialization.Deserializing
{
    ...
    public sealed class ObjectFactory
    {
        ...
        public ObjectFactory(IComplexValueConverter complexValueConverter, bool ignoreReadOnlyProperties = false)
        {
        ...
        private object createObjectFromComplexProperty(ComplexProperty property)
        {
            ...
            if (property.Type.GetConstructors().Any(c => c.GetParameters().Length == 0))
            {
                obj = Tools.CreateInstance(property.Type);
            }
            else if (complexValueConverter != null)
            {
                obj = complexValueConverter.ConvertFromType(property.Type);
            }
            else
            {
                // let it throw appropriate erro
