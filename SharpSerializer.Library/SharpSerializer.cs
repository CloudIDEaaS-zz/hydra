...
namespace Polenter.Serialization
{
    ...
    public sealed class SharpSerializer
    {
        ...
        private int depth;
        private bool useObjectNaming;
        private IComplexValueConverter complexValueConverter;
        ...
        public PropertyProvider PropertyProvider
        {
            get
            {
                if (_propertyProvider == null) _propertyProvider = new PropertyProvider(ignoreReadOnlyProperties);
        ...
        private void initialize(SharpSerializerXmlSettings settings)
        {
            ...
            complexValueConverter = settings.AdvancedSettings.ComplexValueConverter;
            ...
            var reader = new DefaultXmlReader(typeNameConverter, simpleValueConverter, complexValueConverter, xmlReaderSettings);
            ...
            useObjectNaming = settings.UseObjectNaming;
            ignoreReadOnlyProperties = settings.IgnoreReadOnlyProperties;
            complexValueConverter = settings.AdvancedSettings.ComplexValueConverter;

            if (settings.UseObjectNaming)
            {
                _serializer = new XmlObjectNamingPropertySerializer(writer);
            }
            else
            {
                _serializer = new XmlPropertySerializer(writer, complexValueConverter);
            }

            _deserializer = new XmlPropertyDeserializer(reader, complexValueConverter);
        }
        ...
        public void Serialize(object data, Stream stream)
        {
            ...
            var factory = new PropertyFactory(PropertyProvider, complexValueConverter, depth, skipErrors, ignoreReadOnlyProperties);

            if (useObjectNaming)
            {
                this.RootName = data.GetType().Name;
            }
        ...
        public object Deserialize(Stream stream)
        {
            try
            {
                ...
                var factory = new ObjectFactory(complexValueConverter, ignoreReadOnlyProperties);
